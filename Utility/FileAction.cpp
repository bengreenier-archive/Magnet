...
void util::WriteTo(std::string Filename,std::string Msg)
{
...
std::string util::ReadFrom(std::string Filename)
{
    std::string result="";
    std::string line;
...
bool util::FindFile(std::string file_path){
...
bool util::FindDir(std::string dir)
{
    DIR *pDir;
    bool bExists = false;
...
bool util::Find(std::string path){
    if(FindFile(path)){
        return true;
    }
...
directory_tree_t util::CreateDirectoryTree(std::string path)  throw(Exception){
...
FileType util::GetFileType(std::string file_type)throw(Exception){
    if(!Magnet::IsInitialized()){
        throw Exception("sync_error", "magnet is not initialzied, config could not be accessed");
    }
...
void util::GrowDirectoryTree(directory_tree_t& tree, dir_node* cur_node) throw(util::Exception){
    if(cur_node == NULL){
        cur_node = tree.back();
    }
...
void util::ExplodeString(std::string base, std::string delim, std::vector<std::string>& str_store){
